function(install_qt_lib)
    foreach(module ${ARGV})
        set(target_name Qt6::${module})
        set(module_name Qt6${module})

        # Get full path to some known Qt library (i.e. /path/to/lib/libQt6Widgets.so.5.5.1)
        get_target_property(target_path Qt6::Core LOCATION)

        # Get folder path of library (i.e. /path/to/lib)
        get_filename_component(qt_lib_dir ${target_path} DIRECTORY)

        if (SYSTEM_LINUX)
            # Not very good solution, but we simply take all files with *Qt6<module>* in names (including symlinks)
...
function(install_icu_libs)
    ...
    # For this we take some known target (Qt6::Core in this case)
    # and taking path to this library.

    # Get full path to known library (i.e. /path/to/lib/libQt6Core.so.5.5.1)
    get_target_property(target_path Qt6::Core LOCATION)
...
# Installs Qt6 plugins. Uses value of global variable "qt_plugins_dir" as
# absolute path to where install plugins.
#
# You can find list of plugin names by examing this variable: Qt6<Module>_PLUGINS
# For example, Qt6Gui_PLUGINS.
#
...
function(install_qt_plugins)
    foreach(name ${ARGV})
        ...
        # Get absolute path to parent folder (i.e. /path/to/plugins/imageformats for Qt6::QGifPlugin)
        get_filename_component(plugin_dir ${target_path} DIRECTORY)

        # Get plugin parent directory (i.e. "imageformats" for Qt6::QGifPlugin)
        get_filename_component(plugin_dir_name ${plugin_dir} NAME)

        # Get plugin file name (i.e. "libqgif.so" for Qt6::QGifPlugin)
